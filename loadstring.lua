-- ReaperHub v2.5 - Premium Executor Hub
-- discord.gg/reaperhub

-- Redesigned with Futuristic Glassmorphic Design

--[[
    ReaperHub v2.5 (Glassmorphic Futuristic Edition)
    Premium executor hub with advanced glassmorphic UI
...
getgenv().BRAINROT_CONFIG = {
        ...
        -- Carpet Script Features
        CARPET_SPEED_ENABLED = false,
        CARPET_JUMP_ENABLED = false,
}
...
local COLORS = {
    ...
    BackgroundTransparency = 0.25,  -- More transparent for glass effect
    Surface                = Color3.fromRGB(25, 30, 45),
    SurfaceTransparency    = 0.20,  -- Translucent surfaces
    
    -- Text Colors
    Text    = Color3.fromRGB(255, 255, 255),
    TextDim = Color3.fromRGB(180, 200, 220),
    
    -- Futuristic Accent Colors (Cyan/Blue/Purple theme)
    Accent  = Color3.fromRGB(0, 200, 255),      -- Bright cyan
    Purple  = Color3.fromRGB(150, 100, 255),    -- Vibrant purple
    Pink    = Color3.fromRGB(255, 100, 200),    -- Neon pink
    Cyan    = Color3.fromRGB(0, 255, 255),      -- Electric cyan
    Yellow  = Color3.fromRGB(255, 220, 100),    -- Soft yellow
    Blue    = Color3.fromRGB(50, 150, 255),     -- Bright blue
    Red     = Color3.fromRGB(255, 50, 100),     -- Neon red
    
    -- Status Colors
    Success = Color3.fromRGB(0, 255, 150),      -- Neon green
    Warning = Color3.fromRGB(255, 180, 0),      -- Bright orange
    Error   = Color3.fromRGB(255, 50, 100),     -- Neon red
    
    -- Glass Effect Colors
    GlassHighlight = Color3.fromRGB(255, 255, 255),
    GlassShadow    = Color3.fromRGB(0, 0, 0),
}

-- Enhanced Desync V4 Flags - Optimized for maximum performance and no lagback
local DESYNC_FLAGS = {
    -- Core Replicator Settings (Enhanced)
    ...
    -- Network Optimization (V4 Enhanced)
    GameNetDontSendRedundantNumTimes = 0,  -- Reduced for better sync
    GameNetDontSendRedundantDeltaPositionMillionth = 0,  -- Optimized
    GameNetPVHeaderLinearVelocityZeroCutoffExponent = -10000,  -- Increased range
    GameNetPVHeaderRotationalVelocityZeroCutoffExponent = -10000,  -- Increased range
    
    -- Timestep Multipliers (Maximum Performance)
    MaxTimestepMultiplierAcceleration = 2147483647,
    MaxTimestepMultiplierBuoyancy = 2147483647,
    MaxTimestepMultiplierContstraint = 2147483647,
    SimExplicitlyCappedTimestepMultiplier = 2147483647,  -- Increased
    
    -- Interpolation Thresholds (Ultra-Low for smooth desync)
    InterpolationFrameVelocityThresholdMillionth = 1,  -- Reduced
    InterpolationFrameRotVelocityThresholdMillionth = 1,  -- Reduced
    InterpolationFramePositionThresholdMillionth = 1,  -- Reduced
    
    -- Physics Sender Optimization
    PhysicsSenderMaxBandwidthBps = 10000,  -- Reduced for better desync
    S2PhysicsSenderRate = 10000,  -- Reduced for better desync
    ServerMaxBandwith = 30,  -- Reduced bandwidth
    
    -- Velocity & Position Checks (Minimal)
    ...
    CheckPVCachedVelThresholdPercent = 1,  -- Minimal check
    CheckPVCachedRotVelThresholdPercent = 1,  -- Minimal check
    CheckPVLinearVelocityIntegrateVsDeltaPositionThresholdPercent = 0,
    
    -- Arbiter Settings (Enhanced)
    TimestepArbiterVelocityCriteriaThresholdTwoDt = 2147483647,  -- Maximized
    TimestepArbiterHumanoidTurningVelThreshold = 0,  -- Disabled
    TimestepArbiterOmegaThou = 2147483647,  -- Maximized
    
    -- Stream & Memory (Optimized)
    StreamJobNOUVolumeCap = 2147483647,
    StreamJobNOUVolumeLengthCap = 2147483647,
    SimOwnedNOUCountThresholdMillionth = 2147483647,
    MaxMissedWorldStepsRemembered = -2147483648,
    
    -- Replication & Data (Enhanced)
    MaxDataPacketPerSend = 2147483647,
    MaxAcceptableUpdateDelay = 0,  -- Instant updates
    ReplicationFocusNouExtentsSizeCutoffForPauseStuds = 2147483647,
    
    -- Performance Flags
    DisableDPIScale = true,
    WorldStepMax = 60,  -- Increased for smoother performance
    DebugSendDistInSteps = -2147483648,
    AngularVelociryLimit = 720  -- Doubled for better rotation
}
...
local PLAYER_OUTLINE_MAIN = Color3.fromRGB(0, 255, 255)  -- Electric cyan
local PLAYER_OUTLINE_SOFT = Color3.fromRGB(100, 150, 255)  -- Soft blue
...
local carpetJumpActive = false
local carpetBodyVelocity = nil
local carpetBodyGyro = nil
local carpetSpeedConnection = nil
local carpetJumpConnection = nil
local carpetPlatform = nil
local CARPET_SPEED = 150
...
local function applyTitanzButtonStyle(frame)
    ...
    -- Glassmorphic background with subtle tint
    frame.BackgroundColor3 = blendColor(COLORS.Surface, COLORS.Accent, 0.15)
    frame.BackgroundTransparency = 0.35  -- More transparent for glass effect
    ...
    -- Add gradient to border for futuristic effect
    if not border:FindFirstChild("BorderGradient") then
        ...
        task.spawn(function()
            while border.Parent do
                for i = 0, 360, 2 do
                    if not border.Parent then break end
                    gradient.Rotation = i
                    task.wait(0.03)
                end
            end
    ...
    if not frame:FindFirstChild("InnerGlow") then
        local innerGlow = Instance.new("Frame")
        innerGlow.Name = "InnerGlow"
        innerGlow.Size = UDim2.new(1, 0, 1, 0)
        innerGlow.BackgroundColor3 = COLORS.GlassHighlight
        innerGlow.BackgroundTransparency = 0.92
        innerGlow.BorderSizePixel = 0
        innerGlow.ZIndex = frame.ZIndex + 1
...
local function makeTextGlow(textElement, color1, color2, duration, delay)
    color1 = color1 or COLORS.Cyan
    color2 = color2 or COLORS.Purple
...
local NOTIFICATION_TYPES = {
    ...
    default = {
        icon = "•",
        color = COLORS.Accent,
        gradient = Color3.fromRGB(220, 20, 20)
    }
}
...
function showNotification(message, color, duration)
    ...
    makeTextGlow(icon, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(icon, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(messageLabel, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(messageLabel, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(closeButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(closeButton, COLORS.Cyan, COLORS.Purple, 45) 
...
local function enableDesync()
    ...
    showNotification("Desync V4 Enabled - Enhanced Performance", COLORS.Success)
end
        ...
        if (animalData.uid == ESP_BEST_UID) or (animalData.genValue >= 50000000) then
            ...
            if not model then
                ...
                if not rec or rec.part ~= model then
                    ...
                    makeTextGlow(nameLabel, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
                    addTextGradient(nameLabel, COLORS.Cyan, COLORS.Purple, 45) 
...
function UNLOCK_BASE.CreateGUI()
    ...
    local function createNumberButton(number, yPos)
        ...
        makeTextGlow(btn, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(btn, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createToggleButton(parent, text, isEnabled, yPos, callback)
    ...
    makeTextGlow(text, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(text, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(toggleBg, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(toggleBg, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createKeybindButton(parent, text, currentKey, yPos, callback)
    ...
    makeTextGlow(keyButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(keyButton, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createActionButton(parent, text, buttonText, buttonColor, yPos, callback)
    ...
    makeTextGlow(actionButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(actionButton, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createAnimalEntry(animalData, parent)
    ...
    makeTextGlow(name, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(name, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(tpButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(tpButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(favButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(favButton, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createAnimalsTab(parent)
    ...
    makeTextGlow(searchBox, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(searchBox, COLORS.Cyan, COLORS.Purple, 45) 
...
local function createFavoritesTab(parent)
    ...
    makeTextGlow(title, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(title, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(info, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(info, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    local isMobileDevice = S.UserInputService.TouchEnabled and not S.UserInputService.KeyboardEnabled
    ...
    local addInput = createElement("TextBox", {
        ...
        TextSize = isMobileDevice and 16 or 14,
        Font = Enum.Font.Gotham,
        TextXAlignment = Enum.TextXAlignment.Left,
        ClearTextOnFocus = false,
        TextEditable = true,
    ...
    -- Mobile touch support for text input
    if isMobileDevice then
        addInput.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                task.wait(0.05)
                addInput:CaptureFocus()
            end
        end)
    end
    
    makeTextGlow(addInput, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(addInput, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(addButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(addButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    local function refreshFavoritesList()
        ...
        for index, favName in ipairs(FAVORITES) do
            ...
            makeTextGlow(upButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
            addTextGradient(upButton, COLORS.Cyan, COLORS.Purple, 45) 
            ...
            makeTextGlow(downButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
            addTextGradient(downButton, COLORS.Cyan, COLORS.Purple, 45) 
            ...
            makeTextGlow(removeButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
            addTextGradient(removeButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(apply, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(apply, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(toggleCircle, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(toggleCircle, COLORS.Cyan, COLORS.Purple, 45) 
    applyTitanzButtonStyle(toggleCircle)
    applyTitanzTextStyle(toggleCircle)
    createElement("UICorner", {
        CornerRadius = UDim.new(1, 0),
...
-- ============================================================
-- ============================================================
-- SECTION 21.5: CARPET SCRIPT FUNCTIONS
-- ============================================================

local function createCarpetMovement()
    local character = S.LocalPlayer.Character
    if not character then return end
    local rootPart = character:FindFirstChild("HumanoidRootPart")
    if not rootPart then return end
    
    carpetBodyVelocity = Instance.new("BodyVelocity")
    carpetBodyVelocity.MaxForce = Vector3.new(50000, 0, 50000)
    carpetBodyVelocity.Velocity = Vector3.new(0, 0, 0)
    carpetBodyVelocity.P = 1250
    carpetBodyVelocity.Parent = rootPart
    
    carpetBodyGyro = Instance.new("BodyGyro")
    carpetBodyGyro.MaxTorque = Vector3.new(0, 50000, 0)
    carpetBodyGyro.P = 5000
    carpetBodyGyro.D = 500
    carpetBodyGyro.Parent = rootPart
    
    carpetSpeedConnection = S.RunService.Heartbeat:Connect(function()
        if not carpetSpeedActive or not rootPart then return end
        ...
        if moveDirection.Magnitude > 0 then
            ...
            local targetVelocity = moveDirection * CARPET_SPEED
            carpetBodyVelocity.Velocity = carpetBodyVelocity.Velocity:Lerp(targetVelocity, 0.3)
            
            local targetCFrame = CFrame.new(rootPart.Position, rootPart.Position + moveDirection)
            carpetBodyGyro.CFrame = carpetBodyGyro.CFrame:Lerp(targetCFrame, 0.2)
        else
            carpetBodyVelocity.Velocity = carpetBodyVelocity.Velocity:Lerp(Vector3.new(0, 0, 0), 0.5)
        end
    end)
end

local function removeCarpetMovement()
    if carpetBodyVelocity then carpetBodyVelocity:Destroy() carpetBodyVelocity = nil end
    if carpetBodyGyro then carpetBodyGyro:Destroy() carpetBodyGyro = nil end
    if carpetSpeedConnection then carpetSpeedConnection:Disconnect() carpetSpeedConnection = nil end
end

local function createCarpetPlatform()
    if carpetPlatform then carpetPlatform:Destroy() end
    
    carpetPlatform = Instance.new("Part")
    carpetPlatform.Size = Vector3.new(8, 0.5, 8)
    carpetPlatform.Transparency = 1
    carpetPlatform.Anchored = true
    carpetPlatform.CanCollide = true
    carpetPlatform.Name = "CarpetJumpPlatform"
    carpetPlatform.Parent = workspace
end

local function enableCarpetInfiniteJump()
    createCarpetPlatform()
    
    carpetJumpConnection = S.RunService.Heartbeat:Connect(function()
        if not carpetJumpActive or not carpetPlatform then return end
        ...
        if humanoid:GetState() == Enum.HumanoidStateType.Freefall or 
           ...
           humanoid.FloorMaterial == Enum.Material.Air then
            carpetPlatform.CFrame = CFrame.new(rootPart.Position.X, rootPart.Position.Y - 3, rootPart.Position.Z)
            carpetPlatform.CanCollide = true
        else
            carpetPlatform.CFrame = CFrame.new(rootPart.Position.X, rootPart.Position.Y - 5, rootPart.Position.Z)
            carpetPlatform.CanCollide = false
        end
    end)
end

local function disableCarpetInfiniteJump()
    if carpetJumpConnection then carpetJumpConnection:Disconnect() carpetJumpConnection = nil end
    if carpetPlatform then carpetPlatform:Destroy() carpetPlatform = nil end
end

local function toggleCarpetSpeed()
    carpetSpeedActive = not carpetSpeedActive
    CONFIG.CARPET_SPEED_ENABLED = carpetSpeedActive
    saveConfig()
    
    if carpetSpeedActive then
        createCarpetMovement()
        showNotification("🚀 Carpet Speed: ON", COLORS.Success)
    else
        removeCarpetMovement()
        showNotification("🚀 Carpet Speed: OFF", COLORS.TextDim)
    end
    
    return carpetSpeedActive
end

local function toggleCarpetJump()
    carpetJumpActive = not carpetJumpActive
    CONFIG.CARPET_JUMP_ENABLED = carpetJumpActive
    saveConfig()
    
    if carpetJumpActive then
        enableCarpetInfiniteJump()
        showNotification("🦘 Infinite Jump: ON", COLORS.Success)
    else
        disableCarpetInfiniteJump()
        showNotification("🦘 Infinite Jump: OFF", COLORS.TextDim)
    end
    
    return carpetJumpActive
end

local function createCarpetTab(parent)
    local container = createElement("Frame", {
        Name = "CarpetTab",
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
    ...
    local function addHeader(text)
        ...
        makeTextGlow(headerText, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(headerText, COLORS.Cyan, COLORS.Purple, 45)
    end
    
    addHeader("🚀 CARPET MOVEMENT")
    
    createToggleButton(scroll, "Speed Boost (WASD)", CONFIG.CARPET_SPEED_ENABLED, 0, function()
        return toggleCarpetSpeed()
    end).LayoutOrder = nextOrder()
    
    createToggleButton(scroll, "Infinite Jump", CONFIG.CARPET_JUMP_ENABLED, 0, function()
        return toggleCarpetJump()
    end).LayoutOrder = nextOrder()
    ...
    local infoText = createElement("TextLabel", {
        ...
        Position = UDim2.new(0, 10, 0, 10),
        BackgroundTransparency = 1,
        Text = "ℹ️ CARPET INFO\n\n• Speed Boost: Use WASD to fly around\n• Infinite Jump: Jump infinitely in the air\n• Both features work on mobile & PC",
        TextColor3 = COLORS.TextDim,
        TextSize = 13,
        Font = Enum.Font.Gotham,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    makeTextGlow(infoText, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
...
local function createGui()
    ...
    -- Add glassmorphic border with animated gradient
    local mainBorder = Instance.new("UIStroke")
    mainBorder.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    mainBorder.Thickness = 2.5
    mainBorder.Color = COLORS.Cyan
    mainBorder.Transparency = 0.15
    mainBorder.Parent = mainFrame
    
    local borderGradient = Instance.new("UIGradient")
    borderGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, COLORS.Cyan),
        ColorSequenceKeypoint.new(0.33, COLORS.Purple),
        ColorSequenceKeypoint.new(0.66, COLORS.Pink),
        ColorSequenceKeypoint.new(1, COLORS.Cyan)
    })
    ...
    scanLine.Name = "HolographicScanLine"
    scanLine.Size = UDim2.new(1, 0, 0, 2)
    scanLine.Position = UDim2.new(0, 0, 0, 0)
    scanLine.BackgroundColor3 = COLORS.Cyan
    scanLine.BackgroundTransparency = 0.3
    scanLine.BorderSizePixel = 0
    scanLine.ZIndex = mainFrame.ZIndex + 10
    ...
    task.spawn(function()
        while mainFrame.Parent do
            tween(scanLine, TweenInfo.new(3, Enum.EasingStyle.Linear), {
    ...
    -- Add corner accent lights for futuristic effect
    local corners = {
        {pos = UDim2.new(0, 0, 0, 0), anchor = Vector2.new(0, 0)},
        {pos = UDim2.new(1, 0, 0, 0), anchor = Vector2.new(1, 0)},
        {pos = UDim2.new(0, 0, 1, 0), anchor = Vector2.new(0, 1)},
        {pos = UDim2.new(1, 0, 1, 0), anchor = Vector2.new(1, 1)}
    }
    
    for i, corner in ipairs(corners) do
        local light = Instance.new("Frame")
        light.Name = "CornerLight" .. i
        light.Size = UDim2.new(0, 20, 0, 20)
        light.Position = corner.pos
        light.AnchorPoint = corner.anchor
        light.BackgroundColor3 = i % 2 == 0 and COLORS.Cyan or COLORS.Purple
        light.BackgroundTransparency = 0.5
        light.BorderSizePixel = 0
        light.ZIndex = mainFrame.ZIndex + 5
    ...
    -- Mobile/Tablet toggle button (floating)
        ...
        makeTextGlow(toggleButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(toggleButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    -- Add animated particle effect background to title bar
    local particleContainer = createElement("Frame", {
        Name = "ParticleEffect",
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    for i = 1, 8 do
        local particle = createElement("Frame", {
            ...
            BackgroundColor3 = i % 2 == 0 and COLORS.Cyan or COLORS.Purple,
            BackgroundTransparency = 0.3,
            BorderSizePixel = 0,
            ZIndex = titleBar.ZIndex,
        ...
        task.spawn(function()
            ...
            while particle.Parent do
                tween(particle, TweenInfo.new(speed, Enum.EasingStyle.Linear), {
                    Position = UDim2.new(math.random(0, 100) / 100, 0, math.random(0, 100) / 100, 0),
    ...
    makeTextGlow(mainTitle, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(mainTitle, COLORS.Cyan, COLORS.Purple, 45)  

    local mainSub = createElement("TextLabel", {
        Size = UDim2.new(1, -60, 0, 18),
        Position = UDim2.new(0, 15, 0, 35),
        BackgroundTransparency = 1,
        Text = "Premium Executor Hub | discord.gg/reaperhub",
        TextColor3 = COLORS.Pink,
    ...
    makeTextGlow(mainSub, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(mainSub, COLORS.Cyan, COLORS.Purple, 45)  
    ...
    -- Add glassmorphic border to tab bar
    local tabBarBorder = Instance.new("UIStroke")
    tabBarBorder.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    tabBarBorder.Thickness = 1.5
    tabBarBorder.Color = COLORS.Cyan
    tabBarBorder.Transparency = 0.5
    tabBarBorder.Parent = tabBar
    
    local tabButtons = {}
    local tabNames = { "Brainrots", "Priority", "Tuff :3", "Carpet", "Stats" }
    local tabIds = { "animals", "favorites", "actions", "carpet", "stats" }
    ...
    for i, tabName in ipairs(tabNames) do
        ...
        makeTextGlow(tabButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(tabButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    local carpetTab = createCarpetTab(contentFrame)
    ...
    local function switchTab(tabId)
        ...
        if carpetTab then carpetTab.Visible = (tabId == "carpet") end
...
local function createMiniGui()
    ...
    local function createSection(title, order)
        ...
        makeTextGlow(header, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(header, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    local function createToggle(parent, text, configKey, enableFunc, disableFunc, order)
        ...
        makeTextGlow(container, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(container, COLORS.Cyan, COLORS.Purple, 45) 
        applyTitanzButtonStyle(container)
        ...
        makeTextGlow(label, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(label, COLORS.Cyan, COLORS.Purple, 45) 
        ...
        makeTextGlow(switch, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(switch, COLORS.Cyan, COLORS.Purple, 45) 
        ...
        makeTextGlow(knob, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(knob, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    local function createButton(parent, text, color, callback, order)
        ...
        makeTextGlow(button, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
        addTextGradient(button, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    makeTextGlow(headerTitle, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(headerTitle, COLORS.Cyan, COLORS.Purple, 45)  
    ...
    makeTextGlow(minimizeButton, COLORS.Cyan, COLORS.Purple, 1.2, 0.3)
    addTextGradient(minimizeButton, COLORS.Cyan, COLORS.Purple, 45) 
    ...
    createButton(actionsSection, "Tween to Base", Color3.fromRGB(255, 50, 50), function()
        if S.LocalPlayer.Character and S.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local baseName = S.LocalPlayer.Name .. "'s Base"
            local baseLabel = workspace:FindFirstChild(baseName, true)
            if baseLabel and baseLabel:IsA("TextLabel") then
                local basePart = baseLabel.Parent
                if basePart and basePart:IsA("BasePart") then
                    local targetCFrame = basePart.CFrame + Vector3.new(0, 5, 0)
                    local tweenInfo = TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                    local tween = S.TweenService:Create(S.LocalPlayer.Character.HumanoidRootPart, tweenInfo, {CFrame = targetCFrame})
                    tween:Play()
                    showNotification("Tweening to base!", COLORS.Success)
                else
                    showNotification("Base part not found!", COLORS.Error)
                end
            else
                showNotification("Base not found!", COLORS.Error)
            end
        else
