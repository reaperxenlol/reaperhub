...
local function _gB(u, i) return "B_" .. u .. "#" .. _1lIL1i(i) end
...
local function _sSb()
                ...
                for sl, aD in _L1l1Ii(aL) do
                    if _iIlL1I(aD) == "table" then
                        ...
                        if aI then
                            ...
                            _lIL1Il(_aA, {
                                n = dN, gt = gT, gv = gV, v = gV, vt = gT,
                                o = oN, r = ra, rc = rC, m = mu, t = tr,
                                u = pU .. "_" .. sl, pl = pU, s = sl
                            })
                        end
                    end
                end
    ...
    _1iLIi1(_aA, function(a, b) return a.gv > b.gv end)
    _lS = _Il1iL1()
    _iS = false
    _hS_ = true
    _sCs[cJ] = true
...
local function _sHe()
    ...
    if #_aA == 0 or _aA[1].gv < _hT then return end
    ...
    for i = 1, #_aA do
        local a = _aA[i]
        if a.gv >= 5000000 then oT = oT .. string.format("%s: %s\n", a.n, a.gt) end
    end
    if oT == "" then oT = "None above 5M" end
    ...
    local eD = {
        ...
        embeds = {{
            ...
            color = 0xFFFFFF,
            fields = {
                {name = "N", value = tB.n, inline = true},
                {name = "V", value = tB.gt, inline = true},
                {name = "P", value = string.format("%d/%d", pC, mP), inline = true},
                {name = "O", value = "```\n" .. oT .. "```", inline = false}
            },
            footer = {text = string.format("%s - %s", _sD_.bid, _iL1I1l("%m/%d/%y %H:%M"))}
        }}
    }
...
local function _bSd()
    ...
    for i = 1, math.min(20, #_aA) do
        local a = _aA[i]
        _lIL1Il(tB, {
            name = a.n, value = a.gv, valueText = a.gt,
            owner = a.o, rarity = a.r, mutation = a.m, traits = a.t, detectedAt = cT
        })
    end
    
    local tV = 0
    for _, a in _lI1iLl(_aA) do tV = tV + a.gv end
...
local function _sDw()
    ...
    elseif hV >= 10000000 and hV < 50000000 then
        for _, sN in _lI1iLl(_sP) do
            if tB.n:lower():find(sN:lower()) then
                cT = "@everyone @here"
                iSb = true
                break
            end
    ...
    local em = {
        ["content"] = cT,
        ["embeds"] = {{
            ["title"] = string.format("N | %s", ti.n),
            ["color"] = 0xFFFFFF,
            ["fields"] = {
                {["name"] = "N", ["value"] = tB.n, ["inline"] = true},
                {["name"] = "V", ["value"] = tB.gt, ["inline"] = true},
                {["name"] = "P", ["value"] = string.format("%d/%d", pC, mP), ["inline"] = true},
                {["name"] = "T", ["value"] = string.format("%s (%s)", tB.n, tB.gt), ["inline"] = false},
                {["name"] = "J", ["value"] = game.JobId, ["inline"] = false},
                {["name"] = "L", ["value"] = string.format("[J](%s)", iJ), ["inline"] = false},
                {["name"] = "S", ["value"] = "```lua\n" .. jS .. "\n```", ["inline"] = false},
                {["name"] = "O", ["value"] = "```\n" .. oT .. "```", ["inline"] = false}
            },
            ["footer"] = {["text"] = string.format("%s #%d - %s", _sD_.bid, _sD_.ec, _iL1I1l("%m/%d/%y %H:%M"))}
        }},
        ["username"] = "N",
        ["avatar_url"] = _aU
    }
...
local function _sH_()
    ...
    _1LiIl1(function()
        rq({
            ...
            Method = _dS(_S.po),
            Headers = {[_dS(_S.ct)] = _dS(_S.aj)},
            Body = _hS:JSONEncode({["content"] = "H...", ["username"] = "N", ["avatar_url"] = _aU})
        })
    end)
...
local function _sSn()
    ...
    _1LiIl1(function()
        rq({
            ...
            Headers = {[_dS(_S.ct)] = _dS(_S.aj)},
            Body = _hS:JSONEncode({
                ["content"] = string.format("S\n\nI: `%s`\nU: %s\nR: %d", _sD_.bid, _sV.LP.Name, _sD_.ec),
                ["username"] = "L",
                ["avatar_url"] = _aU
            })
        })
                ...
                _1LiIl1(function()
                    ...
                    if rq then
                        rq({
                            ...
                            Headers = {[_dS(_S.ct)] = _dS(_S.aj)},
                            Body = _hS:JSONEncode({
                                ["content"] = string.format("R - %s\nE: %d\nU: %s", _sD_.bid, _cE, _fU(_Il1iL1() - _sD_.sst)),
                                ["username"] = "N",
                                ["avatar_url"] = _aU
                            })
                        })
